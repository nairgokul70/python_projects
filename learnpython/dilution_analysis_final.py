...
def aggregatePriorPostMonths(df_list, cLevel, deltaptcount_vec, event_bindex, event_eindex, event_sum, dic_customer, time_vec, beginEGM_vec, gamesplayed_vec, adjgamesplayed_vec, theowin_vec, adjtheowin_vec, actualwin_vec, adjactualwin_vec, machinedays_vec):
  ...
  if len(event_bindex) > 0:
    for i, value in enumerate(event_sum):
      if event_bindex[i] - preN > 0 and event_eindex[i] + postN + 1 < len(deltaptcount_vec):
         ...
         df_dic_temp['preTheoWin']=sum(theowin_vec[ event_bindex[i] - preN:event_bindex[i] ])
         ...
         df_dic_temp['postTheoWin']=sum(theowin_vec[ event_eindex[i] +1:event_eindex[i] + postN +1 ])
...
##Casino Level query
cur.execute(
"SELECT customernumber, customername, year, month, beginfootprint, deltaptcount, trainset, gamesPlayed, ActualWin, TheoWin, MachineDays, GamesPlayedAdj, ActualWinAdj, TheoWinAdj FROM dz.vw_dilution ORDER BY customernumber, year, month")
results = cur.fetchall()

#Call to method that writes query to file
writeQueryResultsToFile( rawByCustomerFilename )
...
datraw = readQueryResultsFromFile( rawByCustomerFilename )
...
namesdf=['customernumber','customername','starttime','endtime','beginfootprint','afterfootprint','deltaptcount','%EGMchange'
     ,'preUtilization','adjpreUtilization','postUtilization','adjpostUtilization','preTheoWin','preTheoWPUPD','adjpreTheoWPUPD','postTheoWin'
...
writeDFList( df_list, outputFilename = "dilution_events.csv")
...
print df_list[0]
##Function call to write the header (namesdf) and the output vector (df_List) to a file
writeDFList(df_list, outputFilename = "dilutionDenomSeries_events_denom_"+str(denomList).strip('[]').replace(",","_")+"_series_"+str(seriesList).strip('[]').replace(",","_").replace("'","")+".csv")
